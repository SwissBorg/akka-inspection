...
import com.swissborg.akkainspection.FragmentId
...
final case object InspectableActorsRequest extends RequestEvent {
  ...
  def fromGRPC(r: grpc.InspectableActorsRequest): InspectableActorsRequest.type =
    grpcIso.get(r)
...
object ResponseEvent {
  ...
  /**
    ...
    */
  implicit val responseEventSemigroup: Semigroup[ResponseEvent] =
    (x: ResponseEvent, y: ResponseEvent) =>
      (x, y) match {
        ...
        case (x: GroupResponse, y: GroupResponse) =>
          GroupResponse(x.inspectableActors ++ y.inspectableActors)
...
final case class InspectableActorsResponse(inspectableActors: List[String]) extends ResponseEvent {
  ...
  val toGRPC: grpc.InspectableActorsResponse =
    InspectableActorsResponse.grpcIso(this)
}

object InspectableActorsResponse {

  def fromGRPC(r: grpc.InspectableActorsResponse): InspectableActorsResponse =
    grpcIso.get(r)
...
object GroupsResponse {
  ...
  def fromGRPC(r: grpc.GroupsResponse): Option[GroupsResponse] =
    grpcPrism.getOption(r)
  ...
  val grpcPrism: Prism[grpc.GroupsResponse, GroupsResponse] =
    Prism.partial[grpc.GroupsResponse, GroupsResponse] {
      ...
      case GroupsResponse(Right(groups)) =>
        grpc.GroupsResponse(
          grpc.GroupsResponse.Res
            .Groups(grpc.GroupsResponse.Groups(groups.map(_.name)))
        )
...
object FragmentIdsResponse {
  ...
  def fromGRPC(r: grpc.FragmentIdsResponse): Option[FragmentIdsResponse] =
    grpcPrism.getOption(r)
    ...
    Prism.partial[grpc.FragmentIdsResponse, FragmentIdsResponse] {
      case grpc.FragmentIdsResponse(
          grpc.FragmentIdsResponse.Res
            .FragmentIds(grpc.FragmentIdsResponse.FragmentIds(state, ids))
          ) =>
      ...
      case grpc.FragmentIdsResponse(
          grpc.FragmentIdsResponse.Res
            .Error(
              grpc.Error(
                grpc.Error.Error
                  .ActorNotInspectable(grpc.Error.ActorNotInspectable(id))
              )
            )
          ) =>
...
object FragmentsResponse {
  ...
  def fromGRPC(r: grpc.FragmentsResponse): Option[FragmentsResponse] =
    grpcPrism.getOption(r)
    ...
    Prism.partial[grpc.FragmentsResponse, FragmentsResponse] {
      case grpc.FragmentsResponse(
          grpc.FragmentsResponse.Res
            .Fragments(grpc.FragmentsResponse.Fragments(state, fragments))
          ) =>
