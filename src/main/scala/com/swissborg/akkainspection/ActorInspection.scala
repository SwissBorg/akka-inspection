...
trait ActorInspection extends Actor with ActorLogging {
  ...
  type FragmentId = akkainspection.FragmentId

  final def FragmentId(id: String) = akkainspection.FragmentId(id)
  ...
  /**
    ...
    */
  final protected def handleInspectionRequests[S](
      state: String,
      s: S,
      fragments: Map[FragmentId, Fragment[S]]
  ): Receive = {
    ...
    case request: FragmentsRequest =>
      val fragmentIds = request.fragmentIds.foldLeft(Set.empty[FragmentId]) {
        case (fragmentIds, fragmentId) =>
          fragmentIds ++ fragmentId.expand(fragments.keySet)
      }
      ...
      val response = request.respondWith(
        ...
        fragmentIds.foldLeft(Map.empty[FragmentId, FinalizedFragment]) {
          case (fragments0, id) =>
            fragments0 + (id -> fragments
              .getOrElse(id, Fragment.undefined)
              .run(s))
        }
      )
...
private[akkainspection] object ActorInspection {
  ...
  /**
    ...
    */
  final case class FragmentsRequest(
      fragmentIds: List[FragmentId],
      replyTo: ActorRef,
  ...
  /**
    ...
    * @param id                the unique-id of the request/response.
    */
  final case class FragmentsResponse(
      state: String,
  ...
  /**
    ...
    * @param id                the unique-id of the request/response.
    */
  final case class FragmentIdsResponse(
      state: String,
