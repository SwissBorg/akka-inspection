package com.swissborg.akkainspection.laws
...
import cats.Eq
import com.swissborg.akkainspection.grpc.FragmentIdsRequest
...
object arbitrary {
  implicit val arbGRPCFragmentIdsRequest: Arbitrary[grpc.FragmentIdsRequest] =
    Arbitrary(
  ...
  implicit val arbGRPCFragmentIdsResponse: Arbitrary[grpc.FragmentIdsResponse] =
    Arbitrary {
      oneOf(
  ...
  implicit val arbActorNotInspectable: Arbitrary[ActorNotInspectable] =
    Arbitrary(
  ...
  implicit val arbFragmentIdsReponse: Arbitrary[FragmentIdsResponse] =
    Arbitrary(
      arbEither[ActorNotInspectable, (String, List[FragmentId])].arbitrary
        .map(a => FragmentIdsResponse(a))
    )
  ...
  /* --- Cogen instances --- */
  implicit val cogenFragmentIdsRequest: Cogen[FragmentIdsRequest] =
    Cogen.cogenString.contramap(_.actor)

  implicit val cogenActorNotInspectable: Cogen[ActorNotInspectable] =
    cogenString.contramap(_.id)
  ...
  implicit val eqFragmentIdsRequest: Equal[FragmentIdsRequest] =
    (a1: FragmentIdsRequest, a2: FragmentIdsRequest) => a1.actor == a2.actor

  implicit val eqFragmentIdsResponse: Equal[FragmentIdsResponse] =
    (a1: FragmentIdsResponse, a2: FragmentIdsResponse) => a1 == a2
