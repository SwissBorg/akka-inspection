...
import com.swissborg.akkainspection.ActorInspection.FinalizedFragment
import com.swissborg.akkainspection.{ActorInspection, FragmentId}
import com.swissborg.akkainspection.manager.BroadcastActor._
import com.swissborg.akkainspection.manager.state._
...
class ActorInspectorManager extends Actor with ActorLogging {
  ...
  private def inspectableActorsResponses: Receive = {
    ...
    // one to be able to track all the different broadcasts it sent out.
    {
      case ActorInspection.FragmentIdsResponse(state, fragmentIds, originalRequester, id) =>
        id match {
          case Some(id) =>
            originalRequester ! BroadcastResponse(convertFragmentIdsResponse(state, fragmentIds), id)
          case None =>
            originalRequester ! convertFragmentIdsResponse(state, fragmentIds)
        }

      case ActorInspection.FragmentsResponse(state, fragments, originalRequester, id) =>
        id match {
          case Some(id) =>
            originalRequester ! BroadcastResponse(convertFragmentsResponse(state, fragments), id)
          case None =>
            originalRequester ! convertFragmentsResponse(state, fragments)
        }
    }
  }
      ...
      responseTo(r, s, replyTo).value.onComplete {
        case Success(Some(response)) =>
          response match {
            ...
            case GroupsResponse(Left(_)) =>
              broadcaster ! BroadcastRequest.create(r, response, replyTo)
            case FragmentsResponse(Left(_)) =>
              broadcaster ! BroadcastRequest.create(r, response, replyTo)
            case FragmentIdsResponse(Left(_)) =>
              broadcaster ! BroadcastRequest.create(r, response, replyTo)
  ...
  private def _responseTo(
    ...
    request match {
      ...
      case GroupsRequest(id) =>
        OptionT.pure(GroupsResponse(s.groupsOf(id).map(_.toList)))
      case GroupRequest(group) =>
        OptionT.pure(GroupResponse(s.inGroup(group).toList.map(_.toId)))
...
object ActorInspectorManager {
  ...
  /**
    ...
    */
  final case class InspectableActorRef private[akkainspection] (ref: ActorRef) {
