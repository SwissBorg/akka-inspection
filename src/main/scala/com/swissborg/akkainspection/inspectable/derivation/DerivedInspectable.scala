...
import com.swissborg.akkainspection.{FragmentId, Render}
...
object DerivedInspectable extends LowPriorityDerivedInspectable1 {
    ...
    new DerivedInspectable[A] {
      override val fragments: Map[FragmentId, akkainspection.Fragment[A]] =
        inspectableRepr.value.value.fragments.map {
          case (id, Getter(fragment)) =>
            id -> Getter[A](fragment.compose(gen.to))
  ...
  implicit def hconsDerivedInspectable[K <: Symbol, H, T <: HList](
    ...
    new DerivedInspectable[FieldType[K, H] :: T] {
      override val fragments: Map[FragmentId, akkainspection.Fragment[FieldType[K, H] :: T]] = {
  ...
  implicit val hnilDerivedInspectable: DerivedInspectable[HNil] = {
    new DerivedInspectable[HNil] {
      override val fragments: Map[FragmentId, akkainspection.Fragment[HNil]] =
        Map.empty[FragmentId, akkainspection.Fragment[HNil]]
    }
  }
}
